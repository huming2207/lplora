...
mod app {
    ...
    #[task(binds = RADIO_IRQ_BUSY, local = [radio, rf_sw_1, rf_sw_2], shared = [uart_tx_q])]
    fn radio_task(ctx: radio_task::Context) {
        let radio = ctx.local.radio;
        let rfsw_1 = ctx.local.rf_sw_1;
        let rfsw_2 = ctx.local.rf_sw_2;
        let (_, irq) = ctx.local.radio.irq_status().unwrap();
        radio.clear_irq_status(irq).unwrap();
        radio.set_rx(Timeout::from_millis_sat(5000)).unwrap();
    }
